...
def run_web_scanner() -> None:
    """
    Web vulnerability scanner -- 20 checks across 5 categories.

    Always active (runs on every scan, not part of the 20-check menu):
    ...
    Content & Paths
      - Sensitive file/path exposure (44 paths: .env, .git, phpinfo,
      ...
      - Reflected XSS parameter probe (unique canary injection, checks reflection)
      - Well-known files & manifests (security.txt, openid-configuration, etc.)
      - CSRF detection (POST forms missing anti-forgery tokens)
      - SSRF parameter scan (URL-accepting parameters in forms and query strings)
